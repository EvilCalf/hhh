...
struct MaxValue
{
	int Value,Dist,iAngleNumber,AngleNumber;
};
	...
	for(j = 0; j <lHeight; j++)
	{
		for(i = 0;i <lWidth; i++)
		{
			...
			if(pixel == 0)
			{
				...
				{//确定一个参数，逐步算出另一个参数
					double k1=i*cos(iAngleNumber*2*3.14/180.0) + j*sin(iAngleNumber*2*3.14/180.0);//;
					iDist = (int) fabs(k1);//cos(iAngleNumber*2*pi/180.0))i*cos(iAngleNumber*2*pi/180.0) + j*sin(iAngleNumber*2*pi/180.0)
